Feature: Test tutorial 603.Traversing_Linked_Data (Stellio)
  This is feature file of the FIWARE step by step tutorial for Traversing Linked Data (Stellio)
  ...
  Scenario: 01 - Retrieve a known store
    ...
    And    I set the url to "http://localhost:8080/ngsi-ld/v1/entities/urn:ngsi-ld:Building:store001"
  ...
  Scenario: 06 - Update the state of a shelf
    When   I set the "Content-Type" header with the value "application/json"
    And    I set the "Link" header with the value "<https://fiware.github.io/tutorials.Step-by-Step/tutorials-context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json""
    And    I set the url to "http://localhost:8080/ngsi-ld/v1/entities/urn:ngsi-ld:Shelf:unit001/attrs"
  ...
  Scenario: 08 - 02 Reading an entity using the default schema
    When   I set the "Content-Type" header with the value "application/json"
    And    I set the "Link" header with the value "<https://fiware.github.io/tutorials.Step-by-Step/tutorials-context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json""
    And    I set the url to "http://localhost:8080/ngsi-ld/v1/entities/urn:ngsi-ld:Building:store005"
    And    I send a GET HTTP request to that url
    Then   I receive a HTTP "200" response code from Stellio with the body equal to "response603-08.json"
  ...
  Scenario: 09 - 03 Reading an entity using an alternate schema
    ...
    And    I set the url to "http://localhost:8080/ngsi-ld/v1/entities/urn:ngsi-ld:Building:store003"
    And    I send a GET HTTP request to that url
    Then   I receive a HTTP "200" response code from Stellio with the body equal to "response603-09.json"
  ...
  Scenario: 10 - 04 Applying entity expansion/compaction
    ...
    And    I send a GET HTTP request to that url
    Then   I receive a HTTP "200" response code from Stellio with the body equal to "response603-10.json"
