...
#
    Scenario: [1] Checking the Stellio service health
        When  I send GET HTTP request to "http://localhost:8080/ngsi-ld/v1/entities/?type=X"
...
#
    Scenario Outline: [2, 3] Creating an Entity
      ...
      Then I receive a HTTP response with the following Stellio data
        | Status-Code | Location   | fiware-correlator |
        | 201         | <location> | Any               |
...
#
    Scenario: [4] OBTAIN ENTITY DATA BY FQN TYPE
      When  I send GET HTTP request to "http://localhost:8080/ngsi-ld/v1/entities?type=https://uri.fiware.org/ns/dataModels%23Building"
