Feature: test tutorial 202.Introduction to IoT Sensors (Stellio)
  ...
  shell-commands: git checkout NGSI-LD ; ./services create ; ./services ${CB_ENVIRONMENT:-stellio}
  ...
  Scenario: Querying the temperature in the context Broker
    ...
    Then  I receive a HTTP "200" response code from Stellio with the body "05.response.json" and exclusions "05.excludes"
  ...
  Scenario: Req 7 - Test the value of the new device
    ...
    Then  I receive a HTTP "200" response code from Stellio with the body "07.response.json" and exclusions "07.excludes"
  ...
  Scenario: Req 10 -- Read the result of the command by querying the CB
    ...
    And   I set header Link to <http://context/ngsi-context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"
    And   I perform the query request
    Then  I receive a HTTP "200" response code from Stellio with the body "10.response.json" and exclusions "10.excludes"
