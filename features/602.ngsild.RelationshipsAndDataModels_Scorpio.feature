...
#
#   Request 1
#       Note: The expected body in the Tutorial has the following differences with respect to the current Scorpio version:
...
#
    ...
    When I send POST HTTP request to orion-ld at "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Shelf:unit001/attrs"
...
#
    Scenario Outline: [9] ADDING 1-MANY RELATIONSHIP
      When I send POST HTTP request to orion-ld at "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Building:store001/attrs"
      And  With the post header "NA": "NA"
      And  With the body request described in an orion-ld file "<file>"
      Then I receive a HTTP response with the following orion-ld data
        | Status-Code | Location   | Connection | fiware-correlator |
...
#
    Scenario Outline: [11] CREATING COMPLEX RELATIONSHIPS
      When I send POST HTTP request to orion-ld at "http://localhost:9090/ngsi-ld/v1/entities/"
      And  With the post header "NA": "NA"
      And  With the body request described in an orion-ld file "<file>"
      Then I receive a HTTP response with the following orion-ld data
        | Status-Code | Location   | Connection | fiware-correlator |
