Feature: test tutorial 102 NGSI-LD: Working with Context feature (Scorpio)
  ...
  shell-commands: git checkout NGSI-LD ; ./services scorpio
...
# that the necessary prerequisites are in place.
  ...
  Scenario: Checking the Service Health
    When I prepare a GET HTTP request to "http://localhost:9090/version"
    And   I perform the query request
    Then  I receive a HTTP "200" response code

  # Request 2: Create data entities
  Scenario: Create a new data entity
    When  I prepare a POST HTTP request to "http://localhost:9090/ngsi-ld/v1/entities/"
  ...
  Scenario: Obtaining Entity data by FQN Type
    When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities?type=https://uri.fiware.org/ns/dataModels%23Building"
  ...
  Scenario: Obtaining Entity data by ID
    When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Building:farm001"
    And   I set header Accept to application/ld+json
    And   I set header Link to <http://context/ngsi-context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"
    And   I perform the query request
   ...
   Scenario Outline: Several queries
     When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities"
     And   I set header Accept to application/ld+json
