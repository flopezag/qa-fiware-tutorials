Feature: test tutorial 202.Introduction to IoT Sensors (Scorpio)
  ...
  Scenario: Querying the temperature in the context Broker
    When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Device:temperature001?attrs=temperature"
    And   I set header fiware-service to openiot
    And   I set header fiware-servicepath to /
  ...
  Scenario: Req 7 - Test the value of the new device
    When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities/?type=Device"
    And   I set header fiware-service to openiot
    And   I set header fiware-servicepath to /
    And   I set header Link to <http://context/ngsi-context.jsonld>; rel="http://www.w3.org/ns/json-ld#context"; type="application/ld+json"
    And   I perform the query request
  ...
  Scenario: Req 10 -- Read the result of the command by querying the CB
    When  I prepare a GET HTTP request to "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Device:water001"
  ...
  Scenario Outline: Req 14, 15, 16 - Activating things with actuators
    When  I prepare a PATCH HTTP request for "<description>" to "http://localhost:9090/ngsi-ld/v1/entities/urn:ngsi-ld:Device:<device>/attrs/<attr>"
